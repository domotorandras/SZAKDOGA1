...
<head>
    ...
    <title>Kerületek</title>
    <script>window.mapGeoJsonPath = "{{ asset('data/optimisedDataDistrict.geojson') }}";</script>
...
<body>
    ...
    <div id="results" class="results-card" hidden>
        ...
        <div id="resultActions" class="results-actions">
            ...
            <button id="newGame"     class="results-btn" onclick="window.location.href='{{ route('districts') }}'" >Új Játék</button>
            <button id="leaderBoard" class="results-btn" onclick="window.location.href='{{ route('districts.leaderboard') }}'" >Ranglista</button>
            @auth
                <form id="scoreForm" method="POST" action="{{ route('districts.store') }}">
                    @csrf
